...
import ResourcesWithPagination from "./ResourcesWithPagination";
...
export const SearchFilterMenu = ({ data }: SearchFilterMenuProps) => {
    ...
    if (sortBy === "Newest") {
      sortedData = data.slice().sort((a, b) => new Date(b.uploadDate).getTime() - new Date(a.uploadDate).getTime());
    } else if (sortBy === "Oldest") {
      sortedData = data.slice().sort((a, b) => new Date(a.uploadDate).getTime() - new Date(b.uploadDate).getTime());
    }
  ...
  return (
    <div> 
      ...
      <ResourcesWithPagination data={sortedData}/>
    </div>
  );
};
