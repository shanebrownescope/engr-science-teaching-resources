...
const updateCourse = async ({ 
  ...
  try {
    ...
    if (!existingCourseResults || existingCourseResults[0].length === 0) {
    ...
    let updateQuery = `UPDATE Courses_v3 SET courseName = ?`;
    ...
    // Check if any rows were actually affected (results[0] is OkPacket with affectedRows property)
...
//above comment is from the first group. I'm leaving it in for fun :]