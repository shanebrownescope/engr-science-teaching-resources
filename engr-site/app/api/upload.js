...
export default upload.single('file')(async function POST(req, res) {
  try {
    ...
    const { results, error: fileError } = await dbConnect({ query: insertFileQuery, values });
    ...
    if (tags && tags.length > 0) {
      ...
      for (const tag of tags) {
        ...
        const tagValues = [tag];
        const { results: tagResults, error: tagError } = await dbConnect({ query: insertTagQuery, values: tagValues });
        ...
        const { error: fileTagError } = await dbConnect({ query: insertFileTagQuery, values: fileTagValues });
